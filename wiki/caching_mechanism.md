...
Scanning thousands of custom PNG and DDS files every time the game starts can significantly delay the "Press Start" screen. PKCore solves this with the **Texture Manifest Cache**.
...
| **3 (High)** | **Mods** | `PKCore/00-Mods/<ModName>/` | **High Priority Overrides.** Files here ALWAYS win. Multiple mods sorted alphabetically (`zz_` prefix wins). |

**Rule of Thumb:** If you want to force a texture or sound to load with highest priority, put it in `PKCore/00-Mods/`.

---

## 3. High-Performance DDS Loading

For the absolute best performance, PKCore supports pre-compressed **DDS** files across textures, sprites, UI masks, and dialogue portraits.
...
- **Formats Supported**: BC1 (DXT1) for opaque textures, BC3 (DXT5) for textures with transparency, BC7 (with DX10 header support) for high-quality compression.
...
## 5. In-Memory Save Slot Portrait Caching

PKCore features dedicated in-memory sprite and texture caching for **Save Slot Party Member Portraits** (`ShowSaveSlotPartyPortraits`):
- Dynamic lookup caching by character ID and Face ID (`$"{game}_{charId}"`).
- Instantaneous 0ms retrieval after initial load during save/load menu scrolling.
- Prioritizes custom DDS and PNG portraits from `PKCore/Textures/` over vanilla assets.
...
| :--- | :--- |
| **Normal Use** | Keep `EnableTextureManifestCache = false` (or `true` if your mod list is finalized) and `EnableMemoryCaching = true`. |
...
| **Large Mod Packs** | Convert textures and portraits to **DDS** format (BC7 / BC3 / BC1) to eliminate load-time processing and stutter. |
...
PKCore/
├── Cache/
│   └── texture_manifest.xml      # Auto-generated texture index cache
├── Config/                       # Data-driven JSON configuration
│   ├── S2WarAbilities.json
│   ├── PortraitMappings.json
│   ├── PortraitVariants.json
│   ├── DialogOverrides.json
│   ├── S1SpeakerOverrides.json
│   └── S2SpeakerOverrides.json
├── Textures/
...
└── 00-Mods/                      # HIGHEST PRIORITY Mod Packages
    ...
    ├── Better-Launcher-BGM-Mod/  # Launcher BGM replacement
    └── CustomMod/                # User mod packages
```
