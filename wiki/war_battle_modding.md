...
> **Note**: This feature is **Suikoden 2 only** (GSD2). Suikoden 1 uses a different army battle system not governed by this JSON mod.
...
> ⚠️ **Important Note**: These customizations only affect values in actua **war battles** (army vs army enemy encounters). They will NOT appear in Apple's war unit management UI where you organize and view your army roster. The UI shows original game stats, while actual battle encounters use your custom configurations.
...
"54": {
  "name": "Luc",
  "abilities": ["SP_MAGIC_WIND1", "SP_FLYING"]
}
```
...
| Ability Code        | In-Game Name        | Description                                              |
| --------------------- | --------------------- | ---------------------------------------------------------- |
...
| `SP_CRITICAL`       | Critical            | Chance for critical damage                               |
| `SP_SCOUT`          | Scout               | Reconnaissance ability                                   |
| `SP_FOREST_WALK`    | Forest Walk         | Larger are movement through forests                      |
| `SP_MAGIC_WIND1`    | Wind Magic          | Wind-based magic attack                                  |
| `SP_MAGIC_FIRE1`    | Fire Magic          | Fire-based magic attack                                  |
| `SP_MAGIC_THUNDER1` | Thunder Magic       | Lightning-based attack                                   |
| `SP_SEE_THROUGH`    | Shortcut            | Detect deception/traps                                   |
...
| `SP_BODY_GUARD`     | Bodyguard           | Increases Defense and can prevent Death                  |
...
| `SP_FLYING`         | Flying              | Same effect as cavalry's increased area                  |
...
- **Normal Gameplay**: Each character can have **up to 2 abilities**
- **With PKCore Mod**: Each character can have **up to 3 abilities** ⭐
- Use `SP_NONE` to leave slots empty
- Abilities beyond slot 3 are ignored

> **Pro Tip**: Using the 3rd ability slot gives you more tactical flexibility! The mod unlocks this for customization.
...
```json
{
  ...
  "characterAbilities": {
    ...
    "62": {
      "name": "Mazus",
      "abilities": ["SP_MAGIC_FIRE1", "SP_MAGIC_THUNDER1"],
      "attack": 12,
      "defense": 8
    },
    "117": {
      "name": "Jeane",
      "abilities": ["SP_MAGIC_THUNDER1", "SP_MEDICAL1"],
      "attack": 10,
      "defense": 8
    }
  }
...
```json
{
  ...
  "characterAbilities": {
    "3347": {
      "name": "Riou",
      "abilities": ["SP_MOUNT", "SP_CRITICAL"],
      "attack": 13,
      "defense": 12
    },
    "3": {
      "name": "Flik",
      "abilities": ["SP_MOUNT", "SP_THROUGH_ROAD"],
      "attack": 12,
      "defense": 11
    },
    "4": {
      "name": "Viktor",
      "abilities": ["SP_MOUNT", "SP_FLAME_SPEAR"],
      "attack": 14,
      "defense": 10
    }
  }
...
```json
{
  ...
  "characterAbilities": {
    "93": {
      "name": "Juan",
      "abilities": ["SP_MEDICAL2", "SP_CHEAR_UP"],
      "defense": 12
    },
    "113": {
      "name": "Annallee",
      "abilities": ["SP_CHEAR_UP", "SP_MEDICAL1"],
      "defense": 10
    }
  }
}
...
```json
{
  ...
  "characterAbilities": {
    ...
    "12": {
      "name": "George",
      "abilities": ["SP_CRITICAL", "SP_SEE_THROUGH"],
      "attack": 13,
      "defense": 10
    },
    "54": {
...
3. ✅ Ensure character IDs are strings (in quotes): `"3347"` not `3347`
4. ✅ Set `DetailedLogs = true` to see if config is loading
5. ✅ Restart the game completely after editing JSON
...
- `SP_MOUNT` + `SP_CRITICAL` = Maximum damage cavalry
- `SP_MAGIC_FIRE1` + `SP_FLYING` = Versatile mage

**Defensive Combos:**

- `SP_SHINING_SHIELD` + `SP_HP_PLUS` = Ultimate tank
- `SP_SEE_THROUGH` + `SP_MOUNT` = Intel specialist

**Support Combos:**

- `SP_MEDICAL2` + `SP_CHEAR_UP` = Team support
- `SP_THROUGH_ROAD` + `SP_SCOUT` = Tactical control

### Memory Structure

```
WAR_CHARA_TYPE (Character Object)
├── nouryoku[]       ← Abilities array (2 slots)
...
A: No, Suikoden 1 has a different War battle. This feature is Suikoden 2 only.
...
**Q: Can I give a character more than 2 abilities?**
A: In normal gameplay, no—characters are limited to 2 abilities. However, **with PKCore's War Battle mod, you can unlock the hidden 3rd ability slot!** Use the JSON config to assign up to 3 abilities per character.
